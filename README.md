...
宏观策略师和行业分析师**不跟着这条交易日级联走**，单独排在每周六——它们既
不下单也不需要盯着开盘时间，放在周末跑，读到的是完整一周的信息（周一到周五
每天的新闻追踪都已经落库），周一早上的交易日级联开场时看到的就是新鲜出炉的
评审结论，而不是"上周五之前"的旧快照。
...
```bash
cd auto_stock_trading_agents      # 项目根目录
source .venv/bin/activate         # 激活后 python/pip/ats 都指向这个虚拟环境
```
