...
## 12. 报告返工：每层一份、结论先行、证据可核对

> 2026-08-20 首跑后用户审阅产出发现的四个缺口（design D17/D18/D19）。
> 前 11 组的代码不回退——本组只改**产出形态**与**渲染内容**，判读逻辑一行不动。

- [x] 12.1 `LayerVerdict` 新增 `candidate_claims`（候选追踪议题：陈述 + 谁能作证 + 什么读数会证伪）；`LayerVerdictView` 同步，字段描述里写明「不参与本期任何计算」
- [x] 12.2 `layer_review` 保留每层的 `ClaimAssessment` 列表（供报告渲染），而不是只留 LLM 写的一行归因；`assemble` 暴露可复用的取数（`_demand_lines` / `_pricing_lines` 的数据源），**判读逻辑不动**
- [x] 12.3 `report.py` 新增层报告渲染，五节且**结论先行**：① 本层结论（层配置+预算+周期+逐票建议权重/stance 表，自足到可据以决策）② 议题证据链 ③ 截面明细 ④ 逐票取舍理由 ⑤ 候选追踪议题
- [x] 12.4 第②节逐条 common 议题渲染：结论、依据强度（仅自述/证据薄）、证人覆盖率、独立证据簇、立场类别数、支持/反驳累计、**未发声的已声明证人**、逐句判读（说话人·维度·理由）
- [x] 12.5 第③节渲染 **relative 逐家读数表**（位置·依据强度·说话人·判读理由）与量化因子明细并列；本层无 relative 命题时明确写「排序仅由量化因子决定，不得当作竞争位置判断」
- [x] 12.6 层报告写入：`_run_layered` 为每个产出结论的层写一份 `层分析-<sector标签>-<层标签>-<日期>.md`；`--no-report` 仍可关闭
- [x] 12.7 `cross_section.write_report` 不再被 CLI 调用：`ats sector crosssection` 只打印表格；确认没有第二处仍在写「截面选股-<层>.md」
- [x] 12.8 总报告收窄：regime + 轮动 + 矛盾 + 缺失层 + **八层结论索引**（配置/信心/预算/一句依据 + 指向层报告），不再重复证据明细
- [x] 12.9 `layer-analyst/SKILL.md` 增加候选议题一节：必须说明**谁能作证**与**什么读数会证伪**，说不出就不提；并明确它不参与本期任何计算
- [x] 12.10 单测：每层恰好一份报告文件、首节含配置与逐票权重、②节含沉默证人、③节含逐家读数、无 relative 时的提示、候选议题不影响权重
- [x] 12.11 清理迁移期污染文件：删除 `截面选股-AI硬件-L3_dc_infra-2026-08-20.md` 与 `截面选股-AI硬件-L4_chip_design-2026-08-20.md`（内容用的旧分层，来自跨在配置写盘两侧的那次跑）；**更早日期的旧层键文件是真历史，保留**
- [x] 12.12 重跑一次周度评审，核对：8 份层报告 + 1 份总报告、层键全为当前键、L7/L8 也有报告、第②③节内容齐备
