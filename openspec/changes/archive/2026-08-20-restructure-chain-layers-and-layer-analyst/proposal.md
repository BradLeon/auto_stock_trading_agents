...
### 四、报告形态：每层一份（含截面）+ 一份跨层总报告

此前每层的信息散在两处：`截面选股-<层>.md`（确定性排序）与总报告里的一个章节
（配置结论）。读的人要回答「这层给多少钱 + 买谁」得在两份文档间来回对照，而**预算那个数
只有在配置结论出来后才确定**——截面报告里的 `layer_cap` 单独看是半成品。
...
**结论先行**——两个「给多少钱 / 买谁」的答案并排放在最前，解释逻辑一律在后：

| 节 | 内容 | 性质 |
|---|---|---|
| 一、**本层结论** | 层配置（超配/标配/低配/清仓）+ 预算 + 周期 + **逐票建议权重与 stance 表** | **结论** |
| 二、议题证据链 | 逐条 common：结论、覆盖率、独立证据簇、立场类别、支持/反驳、**沉默证人**、逐句判读 | 解释「为什么是这个配置」 |
| 三、截面明细 | z 分因子表 + **relative 逐家读数**（位置·依据强度·说话人·理由） | 解释「为什么是这个排序」 |
| 四、逐票取舍理由 | 每只票的依据与分歧、仅自述标记 | 解释「为什么选它」 |
| 五、候选追踪议题 | 分析师自发提出，仅阅读 | 下一轮的输入 |

第一节要能**单独看完就下单**：层给多少钱、层内怎么分，都在那张表里。后面四节是给要核对的人
准备的——**没有它们就只是一个断言**，但把它们放在结论前面等于每周先读三千字再看到答案。

**两个此前只喂给提示词、没喂给读者的东西现在要渲染出来**：

- **议题证据链**：`ClaimAssessment` 里现成的覆盖率、独立证据簇、立场类别数、支持/反驳分、
  **沉默证人**、以及逐句判读（谁说的·哪个维度·理由）。此前报告里每条议题只有一行归因，
  读的人无法核对那个结论是怎么来的。
- **relative 逐家读数**：`entity_readings` 的位置/依据强度/说话人/判读理由表。此前截面报告
  即使开了 `--structure` 也只显示 `moat_pricing` 的**分数**加一行 rationale，
  支撑那个分数的读数从未出现。

**⚠️ 周度评审此前根本不写截面报告**：`write_report` 只在 CLI 的 `sector crosssection`
里被调用，所以每周跑完只有总报告，各层截面文档全靠手动补。这是本次一并修的缺口。
`sector crosssection` 从此只打印不写文件（它是调试工具），保证**一层一份文件**。

### 五、候选追踪议题（只进报告，不落库）

层级分析师在读完本层全部素材后，提出**尚未预设但值得追踪**的议题。与既有的 induction
引擎分工写死：

| | 触发 | 素材 | 出口 |
|---|---|---|---|
| induction 引擎 | 确定性（未映射观测积累到阈值） | 未映射观测 | `claim_proposals` + 人工采纳 |
| 层级分析师 | 语义（它读到了什么） | 本层全部上下文 | **仅报告** |

**不落库**是本次的选择：轻，但代价是它们不去重、不被跟踪——下周可能再提一遍相似的，
而你无从知道哪些已经看过且否决了。若日后觉得值得跟踪，再接进 `claim_proposals`。
两者都**只提议**，只有人能把命题写进 yaml（`CHAIN_EVIDENCE.md` 的既有纪律不变）。

### 六、配置结论绑定仓位（只能在 cap 内下调）
...
- `src/ats/agents/sector/`：新增 `layer_review.py`（编排）；`cross_section.py` 接受预算使用率、
  不再自行写文件；`assemble.py` 的 `_chain_evidence` 拆成按层产出；`review.py` 改为消费层级结论
- `src/ats/agents/sector/report.py`：新增每层报告渲染（五节），总报告收窄为轮动 + 索引
- `src/ats/chain/factor_evidence.py`：暴露逐家读数供报告渲染（**判读逻辑不动**）
