...
### 判据小节按标题语义定位，不按节号（2026-08-18 修）

层 1 与层 2 都要先在笔记里找到「判据」那一节。原来两处都是 `split("## 三")`——**把四节
模板当成了对所有笔记节号的承诺**，而没有任何东西保证它。`芯片设计.md` 扩到十五节
（判据在 §八）、`半导体设备.md` 扩到六节（判据在 §四）之后，两个工具都开始静默读错节：

| 工具 | 静默后果 |
|---|---|
| `kb_audit` | 判据数为 0 → 覆盖率那句被整句省略、`uncited` 恒空 → **该层照常打印「无异常」** |
| `kb_perturb` | `ablate` 删掉的是技术曲线，却写上「判据已移除」的标题；`poison` 找不到条目、原样返回 → **一条被标成「投毒」的对照臂**，其零效应会被读成「判据不是 load-bearing」 |

后者更糟：它不是丢失信号，是**制造了这个测试唯一要排除的那个结论**。

现在统一走 `data/industry.criteria_spans()`，按标题里是否含「判据」定位（八份子层笔记写
「护城河判据」、`资本开支链` 写「判据」，而「常见误判」不含），返回**全部**判据小节而不是
第一个。抽不出判据时 `kb_audit` 会报一条 `🟠 判据抽不出` 并让该层 `ok=False`——
**覆盖率指标必须能说出「我什么都没量到」，否则「没量」会被读成「通过」**。

⚠️ 这个 bug 活了这么久，是因为测试夹具里的判据一直在 §三。回归测试现在有两条：一条用
判据在 §五 的合成笔记，一条直接遍历 `structure_notes` 里挂着的**真实笔记**断言每份都抽得
出判据——**笔记一旦被重排出解析器的射程，测试就红**。
