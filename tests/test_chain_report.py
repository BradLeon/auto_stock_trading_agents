...
def test_ai_production_appendix_requires_explicit_dedicated_claim_id():
    """The L1 packet is display-only and must not silently join Chain scoring."""
    base = {
        "status": "ok",
        "claim_id": "ai_core_production_workflow_penetration",
        "claim_text": "production proxy",
    ...
    section = "\n".join(report._ai_production_penetration_section(base))
    assert "AI 应用层：生产化与应用扩散" in section
    assert "不参与产业链命题印证、评分、PEAD、Chief、组合、风险或执行" in section
    assert "46.67%" in section and "snapshot-1" in section
    assert report._ai_production_penetration_section({**base, "claim_id": "wrong"}) == []


def _obs(speaker, concept, *, about=None, direction="up", span="…", metric=""):
    return Observation(
        document_id=f"{speaker}-d",
        entity=(about or speaker).upper(),
        source_entity=speaker.upper(),
        metric=metric or concept or "misc",
        concept=concept,
        period="FY26Q3",
        observation_type="guidance",
        stance="incumbent",
        direction=direction,
        evidence_span=span,
        observed_at=NOW,
...
def test_no_llm_report_is_explicitly_unadjudicated(monkeypatch):
    ...
    md = report.render(
        load_sector_config("ai_hardware"), store, as_of=NOW, ind_cfg={}, allow_llm=False
    )
...
def test_report_surfaces_pool_pending_proposals_and_failures():
    store = get_store()
    store.save_observation(
        _obs("COHR", "", metric="optical_lead_time", span="lead times extended to 26 weeks")
    )
    store.save_claim_proposal(
        ClaimProposal(
            statement="瓶颈正从存储转向光互联", signature="sig", created_at=NOW, layer_hint="L5→L3"
        )
    )
    store.save_observation_failure(
        ObservationFailure(document_id="d1", entity="ORCL", reason="指标未单独披露", at=NOW)
    )

    md = report.render(cfg=load_sector_config("ai_hardware"), store=store, as_of=NOW, ind_cfg={})
...
def test_write_is_idempotent_and_degrades_without_output_dir(tmp_path):
    ...
    assert report.write(cfg.model_copy(update={"output_dir": ""}), store, as_of=NOW) is None
    p1 = report.write(cfg.model_copy(update={"output_dir": str(tmp_path)}), store, as_of=NOW)
    p2 = report.write(cfg.model_copy(update={"output_dir": str(tmp_path)}), store, as_of=NOW)
...
def test_third_party_sources_are_shown_grounded_in_stored_data(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        "load_sources",
        lambda: [
            chain_sources.SourceDef(
                id="tw_ic_exports",
                label="台湾 IC 出口",
                adapter="tw_mof",
                entity="TW_IC_EXPORT",
                stance="regulator",
                observation_type="regulatory",
                cadence="monthly",
...
def test_a_failed_fetch_since_the_last_reading_is_flagged(monkeypatch):
    ...
    old = _obs("TW_IC_EXPORT", "supply_tightness", span="旧读数", metric="tw_ic_exports_mom")
    store.save_observation(old.model_copy(update={"observed_at": NOW - timedelta(days=40)}))
