...
- **共享文档资产**：正文保存在 `信息源/`（或 `ATS_DOCS_ROOT`）下；`source_documents` 保存逻辑文档目录，`document_versions` 保存不可变版本，`document_chunks` 提供文本检索，`document_processing_runs` 区分各 Workflow 的处理状态和版本。
