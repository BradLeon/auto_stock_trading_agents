...
| ONS BICS AI | 英国企业 AI 广度、使用深度、员工日常使用、用途和组织适配 | 生产化深度的补充快照 | ~~**P2：监听后接入**~~ **已于 2026-09-19 正式退役（`retire-ons-bics-ai`），不导出历史数据** |
...
> **已于 2026-09-19 正式退役（`retire-ons-bics-ai`），不导出历史数据。**
> 来源曾以 `ons_bics_ai` 接入并入库（12 个 wave / 5,081 条观测 / 21 个 `BUSINESS_POP:UK:*` 实体），
> 但从未成为任何 L1 Observer、报告或图表的输入。退役原因是信息量与时效性均不达标：深度题
> （extensive / limited / pilot）只在 wave 159 出现过一次，无法形成同口径可重复序列；最近一次探测
> （wave 163）返回 `question_not_fielded`，上游已停止提问。库内观测按本次 change 的显式 purge
> 决策清除。**下文 6.1–6.4 保留为当次定级的研究依据，不再代表当前接入状态**；不得用叙述回填
> 「非美国企业采用」这一已存在的来源空缺。
...
5. **ONS BICS AI**（**已于 2026-09-19 正式退役（`retire-ons-bics-ai`），不导出历史数据**）：方法价值高，但 AI 题不保证每个 wave 存在。先做 release/question watcher，待相同深度题复现后再升级为正式序列。——该 watcher 事实上从未等到第二条可比深度题，故本项以退役收口。
