...
系统 SHALL 为 `ramp_ai_index` 登记独立的十天探测调度意图（`P10D` 或等价的外部 scheduler
表达），不得把 `ai_adoption` 其他来源的频率强行改成十天。每次探测 SHALL 以图表参考期间、
方法/schema 指纹和 payload SHA-256 比较库内最新 accepted 数据；报告查询 SHALL 继续只读
统一结构化库，不因报告运行重复触发网页采集。
