...
| 层 | 硬限额 | 动作 |
|---|---|---|
| 1 标的 | 单票 20% · 每产业链层 weight_cap（如设备层 10%） · 止损 -25% | 削 / block / 强制 trim |
| 2 组合 | 杠杆 1.0 · 现金 ≥5% | 缩买单 |
| 3 因子 | 组合 beta ≤1.5 · 相关簇 ≤75%（实算相关矩阵+聚簇；有意集中故设得高） | block 加仓 |
| 4 回撤 | 回撤 -15% → derisk（只许减仓） · 日亏 -5% → 停新仓 | block 所有新买 |
| 5 压测 | 情景损失 ≤25% NAV（beta×冲击 + AI 泡沫打簇） | block 加重敞口 |
