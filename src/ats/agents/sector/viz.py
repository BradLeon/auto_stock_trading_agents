...
def build_bundle(cfg, review, *, assessments_by_layer: dict, store=None) -> dict:
    ...
    `as_of` date matches the requested date, not just "latest"). `store` is an optional
    injected unstructured-data reader for tests; production opens the released platform
    route and never reads retired evidence tables from Workflow memory.
    """
    if store is None:
        ...
        try:
            return build_bundle(
                cfg, review, assessments_by_layer=assessments_by_layer, store=reader)
        finally:
            reader.close()
