...
- 新增行业分析师配置权契约：只消费 `LayerAnalysis` 投影与共享事实，禁止读取 Macro、Fundamental、Information、Technical 观点；承接层级预算与护栏不变式、跨层轮动；证据冲突须分列保留而不得被单一分数压平，层级投影缺失 / 过期 / schema 不兼容一律按缺失处理并留痕。
- 新增信息分析师角色：输入只能是已准入文档与中性证据，输出 `InformationBrief`（事实变化、影响候选、关联实体、置信度、时效、待核验项），不输出买卖/仓位/组合建议；迁移 `pead/research.py`、`pead/triage.py`、`runtime/digest.py`，并移除 `pead/monitor.py` 直写 dossier 的副作用。每条事实变化须携带事件 / 发布 / 抽取三类时间并做同源聚类（独立信源数不因簇内文档数上调）；该角色具备手动 / 定时 / 文档准入事件三个独立入口，可不依赖其他角色独立终结。
- 基本面拆为例行与事件两种模式：例行更新预期基线，事件在 cutoff 冻结基线并计算对基线 / Consensus / 市场隐含预期的三分差异；**保留非可执行投资观点**（方向、幅度、信心、理由、可证伪条件），剥离数量 sizing 与 `risk assess` / `review_guardrails` / `pre_trade` 调用，payload 不得携带 action 词表取值或数量字段，主理人不得把方向直接映射为交易动作；不再读取 Sector / Macro 观点，跨标的信号链只以中性事实（数据产品或该标的 `InformationBrief`）进入，禁止消费其他标的的基本面结论。
...
- Phase D 只定义触发契约与显式入口（CLI + 显式 `WorkflowRunRequest`），财报日历自动发现与触发留 Phase E。
- 层级分析师独立成包：把 `layer_review.py` 从 `src/ats/agents/sector/` 迁到新建的 `src/ats/agents/layer/`，守卫角色映射改为目录前缀，避免靠文件名约定区分角色。
...
- `agent/information-analyst`：信息分析师只消费已准入文档，产出 `InformationBrief`，禁止投资建议与直连 Provider；三类时间与同源聚类为必需要素，具备独立入口与独立终结。
- `agent/fundamental-pead`：基本面分析师例行 / 事件双模式，剥离越界依赖与内部交易风控，保留非可执行投资观点，跨标的信号只以中性事实进入。
