...
# --------------------------------------------------------------------------- #
# FactSet earnings backdrop (Stage 3)
# --------------------------------------------------------------------------- #
_KEY_METRICS = """
EARNINGS INSIGHT July 24, 2026
Key Metrics
• Earnings Scorecard: For Q2 2026 (with 27% of S&P 500 companies reporting actual
results), 86% of S&P 500 companies have reported a positive EPS surprise and 80% of
S&P 500 companies has reported a positive revenue surprise.
• Earnings Growth: For Q2 2026, the blended (year-over-year) earnings growth rate for
the S&P 500 is 37.9%.
• Earnings Revisions: On June 30, the estimated (year-over-year) earnings growth rate
for the S&P 500 for Q 2 2026 was 23.2%. Nine sectors are reporting higher earnings
today (compared to June 30) due to upward revisions to EPS estimates.
• Earnings Guidance: For Q3 2026, 9 S&P 500 companies have issued negative EPS
guidance and 11 S&P 500 companies have issued positive EPS guidance.
• Valuation: The forward 12 -month P/E ratio for the S&P 500 is 20.1. This P/E ratio is
above the 5-year average (19.9) and above the 10-year average (19.0).
"""


def test_factset_key_metrics_parse():
    from ats.data.factset import parse_key_metrics

    bd = parse_key_metrics(_KEY_METRICS, source="EarningsInsight_072426.pdf")
    assert bd.degraded is False and bd.notes == []
    assert bd.quarter == "Q2 2026"            # "Q 2 2026" in the raw text
    assert bd.fwd_pe == 20.1 and bd.fwd_pe_5y_avg == 19.9 and bd.fwd_pe_10y_avg == 19.0
    assert bd.growth_pct == 37.9 and bd.growth_basis == "blended"
    assert bd.prior_growth_pct == 23.2 and bd.prior_as_of == "June 30"
    assert bd.revision_pp == 14.7             # derived, not parsed
    assert bd.sectors_higher == 9 and bd.revision_direction == "upward"
    assert bd.guidance_quarter == "Q3 2026"   # rolls to the NEXT quarter mid-season
    assert (bd.guidance_negative, bd.guidance_positive) == (9, 11)
    assert (bd.pct_reported, bd.pct_eps_beat, bd.pct_revenue_beat) == (27.0, 86.0, 80.0)
    assert bd.fwd_pe_vs_5y_pct == 1.0


def test_factset_parses_the_pre_reporting_wording_too():
    """Before the quarter reports, FactSet says "estimated", not "blended", and
    omits the Scorecard bullet entirely. A parser tuned to one phase silently
    loses the other for weeks at a time."""
    from ats.data.factset import parse_key_metrics

    early = _KEY_METRICS.replace("blended (year-over-year)", "estimated (year-over-year)")
    early = early.split("• Earnings Scorecard")[0] + early.split("surprise.\n", 1)[1]
    bd = parse_key_metrics(early)
    assert bd.growth_basis == "estimated" and bd.growth_pct == 37.9
    assert bd.pct_reported is None            # absent, not invented
    assert bd.degraded is False               # a missing bullet is not a failure


def test_factset_survives_pypdf_splitting_a_number():
    """pypdf injects stray spaces inside numbers ("was 1 8.8%").

    A naive [\\d.]+ captures "8.8" — a wrong number reaching the model as fact,
    which is worse than a missing one. Observed in EarningsInsight_071026.pdf.
    """
    from ats.data.factset import parse_key_metrics

    bd = parse_key_metrics(_KEY_METRICS.replace("was 23.2%", "was 2 3.2%"))
    assert bd.prior_growth_pct == 23.2


def test_factset_drops_implausible_values_rather_than_passing_them_on():
    from ats.data.factset import parse_key_metrics

    bd = parse_key_metrics(_KEY_METRICS.replace(
        "P/E ratio for the S&P 500 is 20.1", "P/E ratio for the S&P 500 is 201"))
    assert bd.fwd_pe is None                  # 201 is outside [5, 60]
    assert any("超出合理区间" in n for n in bd.notes)
    assert bd.degraded is True                # the anchor field is gone
    # ...but the other fields that parsed cleanly are still kept.
    assert bd.growth_pct == 37.9


def test_factset_degrades_to_prose_when_the_format_changes():
    from ats.data.factset import parse_key_metrics

    bd = parse_key_metrics("FactSet redesigned the report and nothing matches.")
    assert bd.degraded is True and bd.fwd_pe is None
    assert parse_key_metrics("").degraded is True


def test_backdrop_reaches_the_prompt_and_forbids_single_stock_inference():
    from ats.agents.macro import review as macro_review
    from ats.data.factset import parse_key_metrics

    bd = parse_key_metrics(_KEY_METRICS)
    block = macro_review._det_block({
        "quadrant": "transition", "quadrant_state": "insufficient", "quadrant_weeks": 0,
        "growth_axis": 0.0, "inflation_axis": 0.0, "earnings_backdrop": bd})
    assert "前瞻 12 个月 P/E: 20.1" in block and "+14.7pp" in block
    # The role boundary has to travel with the data, not just live in the SKILL.
    assert "不得据此推断任何单个公司的盈利" in block


def test_degraded_backdrop_tells_the_model_not_to_invent_numbers():
    from ats.agents.macro import review as macro_review
    from ats.data.factset import parse_key_metrics

    bd = parse_key_metrics("nothing parses here")
    block = macro_review._det_block({
        "quadrant": "transition", "quadrant_state": "insufficient", "quadrant_weeks": 0,
        "growth_axis": 0.0, "inflation_axis": 0.0, "earnings_backdrop": bd})
    assert "结构化抽取失败" in block and "请勿凭空补数字" in block


def test_report_renders_a_backdrop_even_when_the_indicator_layer_is_empty():
    """FactSet and FRED are different sources; one can be up while the other is
    down, so the report must not gate the backdrop on the indicator layer."""
    from ats.agents.macro import report
    from ats.data.factset import parse_key_metrics
    from ats.schemas.macro_strategy import MacroConfig

    r = MacroReview(name="macro", as_of=datetime(2026, 7, 31, tzinfo=timezone.utc),
                    regime="risk-on", earnings_backdrop=parse_key_metrics(_KEY_METRICS))
    out = report.render(r, MacroConfig(name="macro", label="宏观"))
    assert "总量盈利周期" in out and "20.1" in out and "不含个股" in out
